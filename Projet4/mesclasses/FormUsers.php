<?php 
	...
	class FormUsers extends FormInscription
	{
		...
		public function startFormUser($class, $heading){
 		...
 		public function buttonDeconnexion($url,$name,$value){
 			return ' <form action="'.$url.'" method="post">
 							<input type="submit" name="'.$name.'" value="'.$value.'" class="btn btn-sm btn-danger">
 					 </form>';
 		}
		...
		public function printInfoSimple($class1,$class2, $name, $value){			
			echo $this->formRow($class1).
				'<span>'.$name.'</span>'
				.$this->betweenRowForm($class2).
				'<span>'.$value.'</span>'.
				$this->endFormRow();
		}
